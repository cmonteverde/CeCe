...
""".format(topo_base64=b64encode(open("assets/topography.png", "rb").read()).decode()),
    unsafe_allow_html=True,
)
...
if "chat_history" not in st.session_state:
    st.session_state.chat_history = [
        {
            "role": "assistant",
            "content": "👋Hi there! I’m CeCe, your Climate Copilot. I'm here to help you explore, visualize, and make sense of climate and weather data in a way that’s clear and useful. Whether you want to generate a map, check trends, or just ask a question, I’m here to guide you. Click one of the preset buttons below or start typing in the chat box to begin.",
        }
    ]
...
if st.session_state.show_homepage:
    ...
    with col2:
        if st.button(
            "Launch Climate Copilot", type="primary", use_container_width=True
        ):
...
st.markdown(
    '<div style="padding: 10px 0;"></div>', unsafe_allow_html=True
)  # Reduced padding at top
...
st.markdown(
    '<div class="chat-container" style="margin-bottom: 20px;">', unsafe_allow_html=True
)
...
try:
    ...
    if "map_style" not in st.session_state:
        st.session_state.map_style = (
            "earth_nullschool"  # Set the earth nullschool style as default
        )
    ...
    cols = st.columns([7, 3])
    with cols[1]:
        map_style_idx = st.radio(
            "Map Style",
            options=range(len(map_styles)),
...
st.markdown(
    '<div class="industry-buttons-container" style="display: flex; justify-content: center; flex-wrap: wrap; gap: 15px;">',
    unsafe_allow_html=True,
)

# Generate SVG icons for each industry
industry_icons = {
    "aerospace": """<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
    ...
    "agriculture": """<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
        ...
        C19.5,24.1,19.3,24,19,24c-3,0-13,2-19,8v6c0,0,12-6,18-6c3,0,6,1,8,4h8c0,0,4-8,12-8c3,0,6,1,8,4h8c0,0,4-8,12-8c3,0,6,1,8,4h8c0,0,4-8,12-8V10z"/>
    ...
    "energy": """<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
    ...
    "insurance": """<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
    ...
    "forestry": """<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
    ...
    "catastrophes": """<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
...
st.markdown(
    '<div class="buttons-container" style="margin-top: 10px;">', unsafe_allow_html=True
)
col1, col2, col3, col4, col5, col6 = st.columns(6)
...
st.markdown(
    '<div class="title-text">What would you like CeCe to do for you today?</div>',
    unsafe_allow_html=True,
)
...
if not st.session_state.api_status_checked:
    try:
        ...
        status_code, is_working = test_api_status.check_openai_api_status(
...
placeholder_text = (
    st.session_state.suggested_follow_up
    if "suggested_follow_up" in st.session_state
    else "Give me a breakdown of the weather in San Diego, CA for tomorrow"
)

# Add an HTML div with class for JavaScript targeting
st.markdown(
    "<div class='chat-input-container' id='chat-input-anchor'></div>",
    unsafe_allow_html=True,
)
...
with col2:
    ...
    with st.container():
        ...
        if st.button(
            "⬇",
            help="Scroll to Chat Input",
            key="scroll_input_btn",
            use_container_width=True,
        ):
    ...
    try:
        ...
        messages = [
            ...
            for msg in st.session_state.chat_history[
                1:-1
            ]  # Skip welcome message and exclude latest user message
        ]
...
if st.session_state.active_function == "artistic_maps":
    ...
    location_method = st.radio(
        "Select location input method:", ["City Name", "Coordinates"], horizontal=True
    )
    ...
    fast_mode = st.checkbox(
        "Use fast map mode (recommended for larger date ranges)",
        value=True,
    ...
    if st.button("Generate Precipitation Map"):
        with st.spinner("Fetching precipitation data from NASA POWER API..."):
            try:
                ...
                # Status message
                st.text(
                    f"Fetching precipitation data for {city if location_method == 'City Name' else f'({latitude:.2f}, {longitude:.2f})'} from {start_date_str} to {end_date_str}..."
                )

                # Fetch NASA POWER precipitation data for a region
                # Pass the fast_mode parameter to control data fetching behavior
                precip_df = fetch_precipitation_map_data(
                    latitude,
                    longitude,
                    start_date_str,
                    end_date_str,
                ...
                if map_style == "Felt-Inspired Map Demo":
                    ...
                    st.info(
                        "Using Felt-inspired map with enhanced visualization and interactive features."
                    )

                    # Display the title
                    st.subheader(
                        f"NASA POWER Precipitation Map ({start_date_str} to {end_date_str})"
                    )
                    ...
                    embedded_felt_map.create_embedded_felt_map(
                        lat=latitude, lon=longitude, location_name=location_name
                    )
                    ...
                    st.info(
                        "This is showing a sample visualization of precipitation data. In the next update, we'll integrate the real NASA POWER precipitation data with this modern map style."
                    )
                else:
                ...
                st.info(
                    f"This map shows real precipitation data from NASA POWER API around your selected location for the date range {start_date_str} to {end_date_str}. Data source: NASA POWER (Prediction of Worldwide Energy Resources)."
                )
                ...
                st.warning(
                    "Falling back to simulated precipitation data for demonstration purposes."
                )
                ...
                st.warning(
                    "This is simulated data for demonstration purposes only. It does not represent real precipitation patterns."
                )
    ...
    selected_industry_name = st.selectbox(
        "Select Industry Sector:", industry_names, index=industry_index
    )
    ...
    with col1:
        ...
        if selected_industry == "aerospace":
            ...
            folium.Circle(
                location=[
                    industry_data["center"][0] + 0.2,
                    industry_data["center"][1] - 0.3,
                ],
                radius=8000,
                color="#9370DB",
                fill=True,
            ...
            folium.Circle(
                location=[
                    industry_data["center"][0] - 0.1,
                    industry_data["center"][1] + 0.2,
                ],
                radius=6000,
                color="#FF6347",
                fill=True,
            ...
            folium.Circle(
                location=[
                    industry_data["center"][0] + 0.3,
                    industry_data["center"][1] + 0.1,
                ],
                radius=4000,
                color="#FFD700",
                fill=True,
            ...
            folium.PolyLine(
                locations=[
                    ...
                    [
                        industry_data["center"][0] + 0.5,
                        industry_data["center"][1] + 0.5,
                    ],
                ],
                color="#1E90FF",
                weight=3,
            ...
            folium.PolyLine(
                locations=[
                    [
                        industry_data["center"][0] - 0.6,
                        industry_data["center"][1] - 0.6,
                    ],
                    [
                        industry_data["center"][0] + 0.6,
                        industry_data["center"][1] + 0.6,
            ...
            folium.Circle(
                location=[
                    industry_data["center"][0] + 0.05,
                    industry_data["center"][1] - 0.05,
                ],
                radius=12000,
                color="#9370DB",
                fill=True,
                    ...
                    "features": [
                        {
                            "type": "Feature",
                            "properties": {
                                "risk": "Extreme Risk Zone - 300% premium multiplier"
                            },
                            "geometry": {
                                "type": "Polygon",
                                "coordinates": [
    ...
    with col2:
        ...
        for param in industry_data["parameters"]:
            st.markdown(
                f"<li>{param.replace('_', ' ').title()}</li>", unsafe_allow_html=True
            )
    ...
    with col1:
        ...
        location_method = st.radio(
            ...
            horizontal=True,
            key="resilience_location_method",
        )
        ...
        if location_method == "City Name":
            ...
            if city:
                ...
                if city != st.session_state.last_city:
                    ...
                    if lat and lon:
                        ...
                        if (
                            abs(st.session_state.user_location.get("lat", 0) - lat)
                            > 0.001
                            or abs(st.session_state.user_location.get("lon", 0) - lon)
                            > 0.001
                        ):
                            ...
                            st.session_state.resilience_data_needs_update = True
                            st.info(
                                "Location updated! Climate data will refresh when you generate the report."
                            )
            ...
            if (
                abs(st.session_state.user_location.get("lat", 0) - latitude) > 0.001
                or abs(st.session_state.user_location.get("lon", 0) - longitude) > 0.001
            ):
                st.session_state.user_location = {"lat": latitude, "lon": longitude}
                # Force a refresh of resilience data
                st.session_state.resilience_data_needs_update = True
    ...
    with col2:
        ...
        generate_report = st.button(
            ...
            type="primary",
            key="generate_resilience_report",
        )
    ...
    report = (
        st.session_state.resilience_report
        if "resilience_report" in st.session_state
        and st.session_state.resilience_report
        else None
    )
    ...
    if selected_map_view == "Temperature Change":
        ...
        if report:  # Only show if report is available
            try:
                ...
                radius_km = 50 + (
                    temp_change * 15
                )  # Scale the radius by temperature change
                folium.Circle(
                    location=[latitude, longitude],
                ...
                st.info(
                    "Generate a report first to see temperature change projections on the map."
                )
        ...
        if report:  # Only show if report is available
            try:
                ...
                precip_change = report["climate_projections"]["precipitation"][
                    "change_percent"
                ]
                ...
                radius_km = 50 + (
                    abs(precip_change) * 1
                )  # Scale the radius by precipitation change
                folium.Circle(
                    location=[latitude, longitude],
                    radius=radius_km * 1000,  # Convert to meters
                    color=color,
                ...
                st.info(
                    "Generate a report first to see precipitation change projections on the map."
                )
        ...
        if report:  # Only show if report is available
            try:
                ...
                st.info(
                    "Generate a report first to see sea level rise projections on the map."
                )
        ...
        if report:  # Only show if report is available
            try:
                ...
                radius_km = 40 + (
                    heat_multiplier * 10
                )  # Scale the radius by heat multiplier
                folium.Circle(
                    location=[latitude, longitude],
                    radius=radius_km * 1000,  # Convert to meters
                    color=color,
                ...
                st.info(
                    "Generate a report first to see extreme heat day projections on the map."
                )

    elif selected_map_view == "Industry Risk Zones":
        # Create a visualization specific to the selected industry
        if (
            report and "selected_industry" in locals()
        ):  # Only show if report is available
            try:
                ...
                st.info(
                    "Generate a report first to see industry risk zones on the map."
                )
    ...
    if generate_report:
        with st.spinner(
            f"Generating climate resilience report for {industry_names[selected_industry]} industry in {target_year}..."
        ):
            ...
            try:
                ...
                st.session_state.resilience_report = (
                    climate_resilience.generate_resilience_report(
                        lat=latitude,
                        lon=longitude,
                ...
                st.subheader(
                    f"Climate Resilience Report: {industry_names[selected_industry]} Industry"
                )
                ...
                st.markdown(
                    "<h4 style='color: #1E90FF;'>Climate Projections</h4>",
                    unsafe_allow_html=True,
                )
                ...
                fig.add_trace(
                    go.Indicator(
                        ...
                        delta={
                            "reference": report["climate_projections"]["temperature"][
                                "baseline"
                            ],
                            "relative": False,
                        },
                ...
                fig.add_trace(
                    go.Indicator(
                        mode="number+delta",
                        value=report["climate_projections"]["precipitation"][
                            "projected"
                        ],
                        title={"text": "Precipitation (mm)"},
                        delta={
                            "reference": report["climate_projections"]["precipitation"][
                                "baseline"
                            ],
                            "relative": False,
                        },
                ...
                fig.add_trace(
                    go.Indicator(
                        mode="number",
                        value=report["climate_projections"]["extreme_weather"][
                            "heat_days_multiplier"
                        ],
                ...
                st.markdown(
                    "<h4 style='color: #1E90FF;'>Seasonal Changes</h4>",
                    unsafe_allow_html=True,
                )
                ...
                temp_changes = [
                    report["climate_projections"]["temperature"]["seasonal_changes"][
                        season
                    ]
                    for season in seasons
                ]
                precip_changes = [
                    report["climate_projections"]["precipitation"]["seasonal_changes"][
                        season
                    ]
                    for season in seasons
                ]
                ...
                fig.update_layout(
                    ...
                    yaxis2=dict(
                        title="Precipitation Change (%)", side="right", overlaying="y"
                    ),
                ...
                for impact in report["impact_assessment"]["impact_areas"]:
                    st.markdown(
                        f"<li style='color: white; margin-bottom: 5px;'>{impact}</li>",
                        unsafe_allow_html=True,
                    )
                ...
                # Adaptive strategies
                st.markdown(
                    "<h4 style='color: #1E90FF;'>Recommended Adaptive Strategies</h4>",
                    unsafe_allow_html=True,
                )
    ...
    st.markdown(
        "Get official real-time weather alerts and forecasts for US locations from the National Weather Service."
    )
    ...
    location_method = st.radio(
        ...
        ["City Name", "Coordinates"],
        horizontal=True,
        key="nws_location_method",
    )

    if location_method == "City Name":
        city = st.text_input(
            "Enter US City (e.g., 'Miami, FL', 'Topeka, KS')",
            value=(
                "San Francisco, CA"
                if "last_city" not in st.session_state
                else st.session_state.last_city
        ...
        if city:
            ...
            if lat and lon:
                ...
                st.warning(
                    "Could not find coordinates for this city. Please check the spelling."
                )
                latitude = st.session_state.user_location["lat"]
                longitude = st.session_state.user_location["lon"]
        else:
        ...
        with st.spinner("Contacting National Weather Service..."):
            ...
            if "error" in forecast_data:
                ...
                if "US only" in forecast_data["error"]:
                    st.info(
                        "Note: The NOAA NWS API only covers United States locations."
                    )
            else:
                ...
                if not forecast_df.empty:
                    ...
                    for i, row in forecast_df.iterrows():
                        with st.expander(
                            f"{row['Period']}: {row['Forecast']} ({row['Temperature']})"
                        ):
